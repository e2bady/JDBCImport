...
public final class PrettyPrinter {
    ...
    public static <K,V> String print(Map<K,Collection<V>> table, Printer<K> kPrinter, Printer<V> vPrinter) {
    	String[][] t = new String[table.keySet().size()][];
    	int j = 0;
        for(K key : table.keySet()) {
        	int i = 0;
        	String[] tr = new String[table.get(key).size() + 1];
        	tr[i++] = kPrinter.print(key);
        	for(V val : table.get(key)) {
        		tr[i++] = vPrinter.print(val);
        	}
        	t[j++] = tr;
        }
		return print(t);
